...
class InventoryRepository @Inject constructor(private val http:AxeCloudHttpClient,private val sessions:SessionStore){
 ...
 suspend fun adjust(item:InventoryItem,delta:Int){val s=session();http.patch(api("/api/inventory/${enc(item.id)}"),buildJsonObject{put("tenantId",s.tenantId);put("quantidade_atual",(item.quantity+delta).coerceAtLeast(0))},s.accessToken)}
