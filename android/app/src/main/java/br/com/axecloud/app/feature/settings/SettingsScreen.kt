...
import br.com.axecloud.app.feature.notifications.NativeNotificationPreferences
import br.com.axecloud.app.feature.notifications.NotificationPreferencesStore
...
@Composable private fun NotificationSettings(open:()->Unit){val ctx=LocalContext.current;var prefs by remember{mutableStateOf(NotificationPreferencesStore.read(ctx))};fun update(value:NativeNotificationPreferences){prefs=value;NotificationPreferencesStore.save(ctx,value);NotificationSyncScheduler.kick(ctx)};SettingsPanel("Avisos do aplicativo",Icons.Outlined.NotificationsActive){Text("Escolha o que pode aparecer fora do AxéCloud. Som, vibração e tela bloqueada continuam sob controle do Android.",color=AxeCloudThemeTokens.Muted,fontSize=12.sp);Toggle("Financeiro e mensalidades",prefs.finance){update(prefs.copy(finance=it))};Toggle("Giras e agenda",prefs.agenda){update(prefs.copy(agenda=it))};Toggle("Comunicados da casa",prefs.notices){update(prefs.copy(notices=it))};Toggle("Obrigações e preceitos",prefs.routine){update(prefs.copy(routine=it))};Toggle("Acolhimento e estoque",prefs.management){update(prefs.copy(management=it))};Button(open,Modifier.fillMaxWidth(),colors=ButtonDefaults.buttonColors(containerColor=SettingsBlue)){Text("Som e permissões no Android")};Info("Sincronização em segundo plano","O Android consulta periodicamente as pendências reais e evita repetir avisos.");Info("Push instantâneo","Entra por cima desta central quando o projeto Firebase da Play Store for conectado.")}}
