...
class SettingsViewModel @Inject constructor(private val repo:SettingsRepository):ViewModel(){
    ...
    init{load()}
    fun load()=viewModelScope.launch{m.update{it.copy(loading=true,error=null)};runCatching{repo.load()}.onSuccess{p->m.value=SettingsUiState(false,identity=p.identity,portal=p.portal,subscription=p.subscription,whatsapp=p.whatsapp,plan=p.subscription.plan)}.onFailure{e->m.update{it.copy(loading=false,error=e.message)}}}
    ...
    fun whatsappView(v:String)=m.update{it.copy(whatsappView=v,error=null)}
    fun whatsapp(v:WhatsAppSettings)=m.update{it.copy(whatsapp=v,error=null)}
    fun deleteDialog(open:Boolean)=m.update{it.copy(deleteDialog=open,error=null)}
    fun consume()=m.update{it.copy(message=null)}
    fun save()=action("Configurações salvas."){if(m.value.section=="identidade")repo.saveIdentity(m.value.identity)else{val updated=repo.savePortal(m.value.portal);m.update{it.copy(portal=updated)}}}
    ...
    fun saveWhatsApp(value:WhatsAppPreferences){m.update{it.copy(whatsapp=it.whatsapp.copy(preferences=value))};action("Automações atualizadas."){repo.saveWhatsAppPreferences(value)}}
    fun testWhatsApp(){val phone=m.value.whatsapp.testPhone.filter(Char::isDigit);if(phone.length !in 10..13){m.update{it.copy(error="Informe um celular válido com DDD.")};return};if(!m.value.whatsapp.connected){m.update{it.copy(error="O canal oficial está indisponível no momento.")};return};action("Mensagem de teste enviada."){repo.testWhatsApp(phone)}}
