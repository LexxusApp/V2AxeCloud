  ...
  return (
    <div className="min-h-screen bg-[#080A0D] text-[#F1F5F9]">
      ...
      <main className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
        <a href={ROUTES.contentHub} className="text-sm font-bold text-[#FBBC00] hover:underline">
          ← Hub de conteúdo
        </a>
        ...
        <div className="mt-10 space-y-8">
          {LITURGICAL_CALENDAR_MONTHS.map((month) => (
            <section key={month.month}>
              <h2 className="text-xl font-bold text-[#FBBC00]">{month.month}</h2>
              <ul className="mt-4 space-y-3">
                {month.dates.map((d) => (
                  <li
                    key={`${month.month}-${d.day}-${d.title}`}
                    className="rounded-xl border border-[#1E242B] bg-[#0B0D11] px-4 py-3"
                  >
                    <p className="text-xs font-bold uppercase tracking-wide text-[#64748B]">{d.day}</p>
                    <p className="font-semibold">{d.title}</p>
                    {d.note ? <p className="mt-1 text-sm text-[#94A3B8]">{d.note}</p> : null}
                  </li>
                ))}
              </ul>
            </section>
