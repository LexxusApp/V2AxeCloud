...
export function DemoMuralPanel({ onNotify }: Props) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="font-display text-lg font-bold text-[#F1F5F9]">Mural de avisos</h3>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <label className="flex cursor-pointer items-center gap-2 text-xs text-[#94A3B8]">
              <input
                ...
                className="rounded border-[#1E242B]"
              />
        ...
        <ul className="space-y-3 lg:col-span-2" role="list">
          {sorted.map((a) => (
            <li
              key={a.id}
              className="rounded-2xl border border-[#1E242B] bg-[#13171D] p-4 transition-colors hover:border-[#2F3643]"
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-wider text-zinc-500">{a.autor}</span>
                    <span className="text-[10px] text-zinc-600">{formatDemoDate(a.data)}</span>
                  </div>
                  <h4 className="mt-2 text-sm font-bold text-[#F1F5F9]">{a.titulo}</h4>
                  <p className="mt-1.5 text-xs leading-relaxed text-[#94A3B8]">{a.resumo}</p>
                </div>
                <button
                  type="button"
                  onClick={() => {
                  ...
                  className="shrink-0 rounded p-1 text-zinc-500 hover:text-rose-400"
