#!/usr/bin/env python3
"""Adiciona FOOTER 'Mensagem automática. Não responda.' em todos os modelos Meta (Utility/Marketing)."""
...
def sanitize_component(comp: dict) -> dict | None:
    ...
    if "buttons" in comp and comp["buttons"]:
        ...
        for btn in comp["buttons"]:
            item = {k: v for k, v in btn.items() if k in {"type", "text", "url", "phone_number", "example", "otp_type"}}
            if item:
                buttons.append(item)
        if buttons:
...
def with_footer(components: list[dict]) -> tuple[list[dict], str]:
    cleaned = []
    had_footer = False
    same_footer = False
    for raw in components or []:
        ...
        if item["type"] == "FOOTER":
            ...
            if text.casefold() == FOOTER_TEXT.casefold():
                same_footer = True
...
def main() -> int:
    ...
    for row in sorted(templates, key=lambda x: str(x.get("name") or "")):
        ...
        if status in SKIP_STATUS or category in SKIP_CATEGORIES or name in SKIP_NAMES:
            print("SKIP %s %s %s" % (status, category, name))
            skipped += 1
            continue
        components, action = with_footer(row.get("components") or [])
        if action == "already":
            print("OK %s %s %s" % (status, name, "footer-já-existe"))
            skipped += 1
            continue
        payload = {"components": components, "category": category}
        st, created = graph(token, ver, "POST", tid, payload)
        if created.get("error"):
            print("ERROR %s http=%s %s %s" % (name, st, err_text(created), action))
            errors += 1
            continue
        print("UPDATED %s %s %s success=%s" % (name, action, created.get("success"), created.get("id") or tid))
        updated += 1
