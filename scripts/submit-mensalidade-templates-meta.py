#!/usr/bin/env python3
"""Cria na Meta os templates de mensalidade. Não imprime token."""
        ...
        "components": [
            {
                "type": "BODY",
                "text": (
                    "Olá, {{1}}! A mensalidade de {{2}} no valor de R$ {{3}} já está "
                    "disponível para pagamento no {{4}}.\n\n"
                    "Sua contribuição fortalece a casa. Axé!"
                ),
                "example": {
                    "body_text": [["Maria Silva", "agosto de 2026", "150,00", "Terreiro de Oxum"]]
                },
            }
        ],
    },
        ...
        "components": [
            {
                "type": "BODY",
                "text": (
                    "Olá, {{1}}! Lembramos que sua mensalidade de {{2}} no valor de R$ {{3}} "
                    "ainda está pendente no {{4}}.\n\n"
                    "Quando puder, regularize pelo portal da casa. Axé!"
                ),
                "example": {
                    "body_text": [
                        ["Maria Silva", "08/2026 (venc. 15/08/2026)", "150,00", "Terreiro de Oxum"]
                    ]
                },
            }
        ],
...
def main() -> int:
    ...
    print("WABA_PREFIX=%s VER=%s" % (waba[:6], ver))
    ...
    print("---")
    for template in TEMPLATES:
        row = get_named(token, ver, waba, template["name"])
        if not row:
            print("MISSING %s" % template["name"])
            continue
        print(
            "%-12s %-12s %-6s %s %s"
            % (
