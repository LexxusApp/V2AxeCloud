  ...
  return (
    <img
      ...
      className={cn(
        ...
        compact ? 'h-[3.25rem] max-w-none sm:h-14 md:h-[5.25rem]' : 'h-[3.75rem] max-w-none sm:h-[5.25rem] md:h-[5.75rem]',
        className,
      )}
    />
  );
      ...
      >
        <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-5 px-4 sm:flex-row sm:px-6 sm:items-start">
          ...
          <ul className="flex items-center gap-2" aria-label="Redes sociais oficiais">
            {[
              { href: 'https://www.instagram.com/axecloudoficial/', label: 'Instagram' },
              { href: 'https://www.tiktok.com/@axecloudoficial', label: 'TikTok' },
            ].map(({ href, label }) => (
              <li key={label}>
                <a
                  href={href}
